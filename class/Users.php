...
class Users extends Database
{
	...
	public function login()
	{
		...
		if (!empty($_POST["login"]) && $_POST["email"] != '' && $_POST["password"] != '') {
			...
			if ($isValidLogin) {
				...
				$errorMessage = "Nome de usuário ou senha invalido!";
			}
		} else if (!empty($_POST["login"])) {
			$errorMessage = "Digite o nome de usuario e senha!";
		}
		return $errorMessage;
	}
	...
	public function listUser()
	{
		...
		$sqlQuery = "SELECT id, name, email, create_at, user_type, status 
			FROM " . $this->userTable;
		...
		while ($user = mysqli_fetch_assoc($result)) {
			...
			if ($user['user_type'] == 'admin') {
				$userRole = '<span class="label label-danger">Administrador</span>';
			} else if ($user['user_type'] == 'user') {
				$userRole = '<span class="label label-warning">Cliente</span>';
			}

			$userRows[] = $user['id'];
			$userRows[] = $user['name'];
			$userRows[] = $user['email'];
			$userRows[] = $user['create_at'];
			$userRows[] = $userRole;
			$userRows[] = $status;
	...
	public function getUserDetails()
	{
		if ($this->id) {
			$sqlQuery = "
				SELECT id, name, email, password, create_at, user_type, status 
				FROM " . $this->userTable . " 
				WHERE id = '" . $this->id . "'";
			$result = mysqli_query($this->dbConnect, $sqlQuery);
