...
class Tickets extends Database
{
	...
	public function showTickets()
	{
		...
		if (!isset($_SESSION["admin"])) {
			$sqlWhere .= " WHERE t.user_id = '" . $_SESSION["userid"] . "' ";
		...
		$time = new time;
		$sqlQuery = "SELECT t.id, t.protocol, t.title, t.init_msg as message, t.date, t.last_reply, t.resolved, u.name as creater, d.name as department, u.user_type, t.user_id, t.user_read, t.admin_read
			FROM tickets t 
			LEFT JOIN users u ON t.user_id = u.id 
			LEFT JOIN departments d ON t.department = d.id $sqlWhere ";
		if (!empty($_POST["search"]["value"])) {
			$sqlQuery .= ' (protocol LIKE "%' . $_POST["search"]["value"] . '%" ';
		...
		while ($ticket = mysqli_fetch_assoc($result)) {
			...
			$ticketRows[] = $ticket['protocol'];
			$ticketRows[] = $title;
			$ticketRows[] = $ticket['department'];
			$ticketRows[] = $ticket['creater'];
			$ticketRows[] = $time->ago($ticket['date']);
			$ticketRows[] = $status;
			$ticketRows[] = '<a href="view_ticket.php?id=' . $ticket["protocol"] . '" class="btn btn-success btn-xs update">Detalhes</a>';
	...
	public function createTicket()
	{
		if (!empty($_POST['subject']) && !empty($_POST['message'])) {
			...
			$queryInsert = "INSERT INTO " . $this->ticketTable . " (protocol, user, title, init_msg, department, date, last_reply, user_read, admin_read, resolved) 
	...
	{
		$sqlQuery = "SELECT t.id, t.protocol, t.title, t.user_id, t.init_msg as message, t.date, t.last_reply, t.resolved, u.name as creater, d.name as department 
